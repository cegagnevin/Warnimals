...
/**
 *
 * Classe de test de la classe TransactionModele (modeles).
 *
 */
class TransactionModeleTestCase extends UnitTestCase
{
	/** DAO */
	private $_dao;
	
	/** Modele Transaction à tester */
	private $_mdlTransaction;
	
	/**
	 * Constructeur par défaut. Instancie le DAO et le TransactionModele à tester.
	 */
	...
	/**
	 * Test du constructeur de TransactionModeleTestCase.
	 */
	function test_constructeur_TransactionModeleTestCase()
	{
		//Asserts
		$this->assertNotNull($this->_dao);
		$this->assertNotNull($this->_mdlTransaction);
		$this->assertTrue($this->_dao instanceof DAO);
		$this->assertTrue($this->_mdlTransaction instanceof TransactionModele);
	}
	...
	// -------------------------------- TESTS DE LA CLASSE TRANSACTIONMODELE ---------------------------------------
	
	/**
	 * Test du constructeur du Modele Transaction.
	 */
	function test_constructeur()
	{
		//Création d'un modele Transaction
		$mdl = TransactionModele::getInstance();
	
		//Asserts
		$this->assertNotNull($mdl);
		$this->assertNotNull($mdl->getDAOTransaction());
		$this->assertTrue($mdl->getDAOTransaction() instanceof TransactionDAO);
	}
	...
	function test_createTransaction()
	{
		try
		{
			...
			$prix = 350;
			$vendu = 0;
			$idTransactionCree = $this->_mdlTransaction->mettreEnVenteAnimal($idAnimal, $prix);
	...
	/**
	 * Test de la méthode : retirerDeLaVenteAnimal().
	 */
	public function test_retirerDeLaVenteAnimal()
	{
		try
		{
			...
			$this->_mdlTransaction->retirerDeLaVenteAnimal($idTransaction);
	...
	/**
	 * Test de la méthode : finaliserTransaction().
	 */
	public function test_finaliserTransaction()
	{
		try
		{
			//Creation d'une transaction de test
	...
	public function test_listerTransactionsEnCours()
	{
		try
		{
			...
			//Execution de la méthode
			$transactionReceived = $this->_mdlTransaction->listerTransactionsEnCours();
	...
	public function test_getTransactionById()
	{
		try
		{
			...
			//Execution de la méthode
			$transactionReceived = $this->_mdlTransaction->getTransactionById($idTransaction);
	...
	public function test_isSelling()
	{
		try
		{
			...
			//Execution de la méthode
			$boolReceived = $this->_mdlTransaction->isSelling($idAnimal);
	...
	public function test_getTransactionEnCoursByAnimal()
	{
		try
		{
			...
			//Execution de la méthode
			$transactionReceived = $this->_mdlTransaction->getTransactionEnCoursByAnimal($idAnimal);
